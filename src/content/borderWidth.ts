...
export function convertTokensToCSS({
  ...
  const cssVariables = tokens
    .filter((t) => t.tokenType === TokenType.borderWidth)
    .map((token) =>
...
export function getBorderWidthTokenFiles({
  tokens,
  tokenGroups,
}: {
  tokens: Token[];
  ...
  const borderWidthTokens = convertTokensToCSS({ tokens, tokenGroups });

  let files: OutputTextFile = FileHelper.createTextFile({
    relativePath: "./",
    fileName: "border-width.css",
    content: generateContentForFile({
      cssVariables: borderWidthTokens,
